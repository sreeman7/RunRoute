...
A React MVP for an AI-powered long-distance running agent. Runners can enter a start location, target distance, route type, pace, style preference, and training goal, then compare three generated route options on an interactive Leaflet/OpenStreetMap street map with OSRM street/path-snapped route lines, route reasoning, distance, estimated time, safety, difficulty, elevation, stops, nutrition guidance, sharing, export, and saved routes.
